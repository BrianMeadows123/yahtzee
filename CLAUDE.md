...
To add a new game: write `games/<name>/logic.js` as pure state functions, add a `ROOM_FACTORIES` entry + a client-state shaper in `server.js`, add a message-type branch in the WS handler, and build `public/<name>.html` + `public/<name>.js` following the existing per-page client pattern below. Persistence/stats are opt-in per game — only Yahtzee has them so far. For any drag-to-place/drag-to-move interaction, reuse the Pointer Events approach in `public/battleship.js`/`public/checkers.js` (ghost element + `elementFromPoint`, no `mouseover`/hover involved) rather than click-to-select — it's the one that actually works on touch.
