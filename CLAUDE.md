...
Solitaire is single-player, so there's no opponent to synchronize with and the whole room/seat/WS machinery above doesn't apply to it. Instead, `games/solitaire/logic.js` is served **statically to the browser** via a dedicated route (`server.js` serves anything under `/games/` read-only from the project's `games/` directory, same traversal guard as `PUBLIC_DIR`) and imported directly as a native ES module in `public/solitaire.js` (`<script type="module">`) — the exact same file `node --test` exercises server-side also runs the live game client-side. The server's involvement is three REST endpoints — `GET /api/solitaire/daily-seed`, `POST /api/solitaire/result`, `GET /api/solitaire/stats` — for resolving/persisting the daily challenge and finished games; there's no protocol, no seats, no reconnect logic to write. If a future game is also genuinely single-player, this is the pattern to reuse — don't force it into `ROOM_FACTORIES` just because every other game lives there.

Solitaire has two modes: **Free Play** (unlimited games, random seed, own stats) and a **Today's Challenge** daily mode (seed usually = today's date, so both people see the identical deal — like Wordle). `solitaire_games` in `db.js` has a `UNIQUE(name, mode, seed)` constraint, which is what enforces "one daily attempt per person per day": a free-play game always gets a fresh random seed so it never collides, but a second daily submission for the same name+date is silently ignored (`INSERT OR IGNORE`) and the client is told via the endpoint's `accepted` boolean so it can show a locked "already played today" result screen instead of letting you replay. In-progress games (both modes) persist to `localStorage` only, keyed by mode/seed, so refreshing mid-game resumes correctly — nothing round-trips to the server until a game finishes or is given up.

**The daily deal is guaranteed solvable, backed by a real proof, not a probability.** `games/solitaire/solver.js` is a greedy best-first search (a bucket priority queue keyed on cards-home-then-cards-revealed, not plain DFS — DFS sinks its whole budget into one deep unproductive branch far too easily for this problem) that reuses `logic.js`'s own `newGame`/`draw`/`move`/`legalDestinations` as its step function, so a found solution is a real, replayable move sequence — never a separate rules reimplementation that could drift from the real game. `server.js`'s `ensureDailySeed(date)` tries the plain date against the solver first, then deterministic suffixed fallbacks (`date-2`, `date-3`, ...) until one comes back solvable, and caches whichever seed won in `daily_seeds` (so the search — which can take real seconds — runs once per calendar date, not once per request). Empirically (see the benchmark this was calibrated against) the plain date solves instantly the large majority of the time and a fallback is rarely needed more than once or twice. The solver yields to the event loop periodically mid-search specifically so a same-day computation never freezes other players' live games on the same Node process — verified by confirming a concurrent timer kept firing throughout a real multi-second solve. On the rare day every candidate is exhausted, it falls back to the plain date (logged, not silently pretended-solvable) — the leaderboard's date column may then show a suffixed seed string instead of a plain date, a minor honest cosmetic wrinkle rather than a false guarantee.

### Shared name picker: one identity across every game
`public/name-picker.js` is a plain (non-module) script attached to `window.NamePicker`, included via `<script src="/name-picker.js">` before each page's own script tag on all 5 game pages — deliberately not an ES module so it works identically from Solitaire's `type="module"` page and the other four classic-script pages (a classic script's globals are visible to a later module too). It renders a Brian/Justy/Custom pill picker (`.choice-group`/`.choice-btn` — the same generic classes Solitaire's mode tabs use) and owns one shared localStorage key (`gamenight-name`, mirroring `yahtzee-theme`'s cross-game sharing) — deliberately *not* per-game, since the identity is a cross-game concept. Each of the 4 realtime games calls `syncSavedName(game)` from its `state`-message handler (right after `mySeat`/`isSpectator` are set) to auto-apply the saved name via `setName` the moment a seat is claimed, so picking a name once on any game already shows up on every other one without re-picking — this is genuinely idempotent (only sends when it'd actually change something), so it's safe to call on every single state update.
